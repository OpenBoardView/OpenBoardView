...
if [ $THREADS -lt 1 ]; then
  fmtText 1 "Unable to detect number of threads, using 1 thread."
  THREADS=1
fi
...
echo "$STRCOMPILE $PROJECT using $(fmtText 4 $THREADS) $STRTHREADS ($BUILDTYPE build)"
echo "Extra flags passed to CMake: $COMPILEFLAGS"
cmake $COMPILEFLAGS ..
[[ "$?" != "0" ]] && fmtText 1 "CMAKE FAILED" && exit 1
if `echo "$COMPILEFLAGS" | grep -q "DEBUG"`; then
  make -j$THREADS install
  [[ "$?" != "0" ]] && fmtText 1 "MAKE INSTALL FAILED" && exit 1
else
  make -j$THREADS install/strip
  [[ "$?" != "0" ]] && fmtText 1 "MAKE INSTALL/STRIP FAILED" && exit 1
fi
...
case "$(uname -s)" in
  *Darwin*)
    # Generate DMG
    make package
    [[ "$?" != "0" ]] && fmtText 1 "MAKE PACKAGE FAILED" && exit 1
    ;;
  *)
